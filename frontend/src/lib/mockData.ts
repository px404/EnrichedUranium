...
import { api } from './api';
import { actorToAgent, backendSessionToFrontend } from './adapters';
...
// POST /sessions  (live, Lightning stalled — backend mock-funds) | mock (mock)
export async function createSession(agentId: string, payload: Partial<Session>): Promise<Session> {
  if (getCurrentMode() === 'live') {
    // Caller (SessionNew) passes the required fields via payload.__backendArgs
    // See SessionNew.tsx for how this is used in live mode.
    const args = (payload as unknown as { __backendArgs?: Parameters<typeof api.createSession>[0] }).__backendArgs;
    if (args) {
      const s = await api.createSession(args);
      return backendSessionToFrontend(s, agentId);
    }
    // Fallback if called without backendArgs
    throw new Error('createSession in live mode requires __backendArgs');
  }
  await delay(500);
