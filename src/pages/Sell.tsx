...
import { getIdentityPubkey, useIdentityPubkey } from '@/lib/identity';
...
const Sell = () => {
  ...
  // Reactive — refreshes when the user signs in/out from Dashboard → Settings.
  const sellerPubkey = useIdentityPubkey();
  ...
  // Workflow (from backend semantics):
  //   funded → matched → in_progress → completed
  // The backend has no "shortlisted but unassigned" query yet, so the inbox
  // here shows requests where the seller has been *matched* but hasn't started
  // working. Once the seller submits a result, the request moves to
  // `in_progress` (during submission) and then `completed`.
  const fetchLiveRequests = async () => {
    const pk = getIdentityPubkey();
    if (!pk) return;
    setLiveLoading(true);
    try {
      const [matchedRes, inProgressRes, completedRes] = await Promise.all([
        api.getRequests({ seller_pubkey: pk, status: 'matched' }),
        api.getRequests({ seller_pubkey: pk, status: 'in_progress' }),
        api.getRequests({ seller_pubkey: pk, status: 'completed' }),
      ]);
      setInbox(matchedRes.requests.map(requestToTask));
      ...
      toast({
        ...
        description: e instanceof Error ? e.message : 'Backend offline?',
        variant: 'destructive',
      });
    } finally {
  ...
  }, [isLive, sellerPubkey]); // fetchLiveRequests is recreated each render but stable in behaviour

  // Light polling every 15s in live mode so newly-assigned tasks appear
  // without a manual refresh. Pause when the tab is hidden.
  useEffect(() => {
    if (!isLive || !sellerPubkey) return;
    const tick = () => { if (!document.hidden) fetchLiveRequests(); };
  ...
  const accept = (t: InboxTask) => {
    if (isLive) {
      // Backend has no explicit "accept" endpoint — the request stays in
      // `matched` until we submit. We optimistically move it to In Progress
      // so the seller can paste their result without losing the assignment.
      setInbox(p => p.filter(x => x.id !== t.id));
      setInProgress(p => [t, ...p]);
      toast({ title: 'Task accepted', description: 'Submit your result below.' });
      return;
    }
  ...
  const submit = async (t: InboxTask) => {
    if (isLive) {
      ...
      const raw = (resultTexts.current[t.id] ?? '').trim();
      if (!raw) {
        toast({
          title: 'Result required',
          description: 'Paste a JSON object or plain text before submitting.',
          variant: 'destructive',
        });
        return;
      }

      // Accept either valid JSON or fall back to wrapping plain text.
      ...
      try {
        await api.submitResult(t.id, {
          seller_pubkey: pk,
          output_payload: outputPayload,
        });
        setInProgress(p => p.filter(x => x.id !== t.id));
        setCompleted(p => [t, ...p]);
        delete resultTexts.current[t.id];
          ...
          <div className="grid lg:grid-cols-[1fr_1fr_1fr_280px] gap-4">
            ...
            <Column title="Inbox" count={inbox.length}>
              {inbox.length === 0 && (
                <Empty msg={
                  isLive
                    ? 'No matched requests yet. New assignments appear here automatically.'
