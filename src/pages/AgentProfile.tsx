...
import { ChevronLeft, Send, Zap, Clock, CheckCircle2, TrendingUp, Calendar, Code2, ChevronDown, Wrench, Shield, AlertTriangle, Loader2, KeyRound } from 'lucide-react';
...
import { addStoredRequestId, useIdentityPubkey } from '@/lib/identity';
import type { Agent, Review } from '@/lib/types';

type RequestPhase = 'idle' | 'submitting' | 'pending_payment' | 'funded' | 'matched' | 'in_progress' | 'completed' | 'failed';
...
const AgentProfile = () => {
  ...
  const identityPubkey = useIdentityPubkey();
  ...
  const [testPhase, setTestPhase] = useState<RequestPhase>('idle');
  ...
  // Pre-select first capability and a sensible budget once the agent loads
  useEffect(() => {
    if (!agent) return;
    if (!selectedCap && agent.skills.length > 0) setSelectedCap(agent.skills[0]);
    const cap = selectedCap || agent.skills[0];
    const capPrice =
      agent.pricing.find(p => p.tier === cap)?.sats ??
      agent.pricing.find(p => /single call|per call|per-call/i.test(p.tier))?.sats ??
      agent.pricePerTask;
    if (!budgetInput) setBudgetInput(capPrice || agent.pricePerTask);
  ...
  const openTestModal = () => {
    ...
    if (agent) {
      setBudgetInput(
        agent.pricing.find(p => p.tier === (selectedCap || agent.skills[0]))?.sats ??
        agent.pricePerTask,
      );
    }
  };

  // Cancel any in-flight poll when the modal closes or the page unmounts
  useEffect(() => {
    if (!testOpen && pollAbort.current) {
  ...
  /** Poll `getRequest` until it terminates, then fetch the result payload. */
  const pollUntilDone = async (id: string) => {
    const ticket = { cancelled: false };
    pollAbort.current = ticket;
    for (let i = 0; i < POLL_MAX_ATTEMPTS; i++) {
      ...
      try {
        ...
        setTestPhase(req.status as RequestPhase);
        if (req.status === 'completed') {
          try {
            const result = await api.getResult(id);
            setTestOutput(JSON.stringify(result, null, 2));
          } catch {
            setTestOutput('Result fetched but body could not be parsed.');
          }
          setTestRunning(false);
          return;
        }
        if (req.status === 'timeout' || req.status === 'cancelled') {
          setTestOutput(JSON.stringify({ status: req.status, request_id: id }, null, 2));
          setTestPhase('failed');
          setTestRunning(false);
          return;
        }
      } catch (e) {
        console.error('[agent-profile] poll error:', e);
      }
    }
    ...
    toast({
      ...
      description: 'No completion within the polling window. Check the dashboard later.',
    });
  };
  ...
  const submitRequest = async () => {
    ...
    // ── Mock mode: keep the simulated sandbox call ────────────────────────
    if (!isLive) {
    ...
    // ── Live mode: real backend request ───────────────────────────────────
    if (!identityPubkey) {
      toast({
        title: 'No identity set',
        description: 'Set your pubkey in Dashboard → Settings before sending requests.',
        variant: 'destructive',
      });
      return;
    }
    const cap = selectedCap || agent.skills[0];
    if (!cap) {
      toast({ title: 'Agent has no capabilities', variant: 'destructive' });
      return;
    }
    const budget = Math.max(budgetInput, minBudget);
    if (budget <= 0) {
      toast({ title: 'Budget must be greater than zero', variant: 'destructive' });
      return;
    }

    // Try parsing JSON; otherwise wrap the raw text under `text`.
    ...
    try {
      const req = await api.createRequest({
        buyer_pubkey: identityPubkey,
        capability_tag: cap,
        input_payload: inputPayload,
        budget_sats: budget,
      });
      addStoredRequestId(req.id);
      setTestRequestId(req.id);
      setTestPhase(req.status as RequestPhase);

      // If the backend stalls on Lightning, surface the payment instructions.
      if (req.payment_instructions || req.invoice) {
        toast({
          title: 'Awaiting payment',
          description: req.payment_instructions ?? 'Pay the returned invoice to fund this request.',
        });
      } else {
        toast({ title: 'Request submitted', description: req.id.slice(0, 16) + '…' });
      }

      pollUntilDone(req.id);
    } catch (e: unknown) {
      setTestRunning(false);
      setTestPhase('failed');
      toast({
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([getAgent(agentId), getReviews(agentId)])
      .then(([a, r]) => {
        setAgent(a ?? null);
        setReviews(r);
      })
    ...
    <Layout>
        ...
        <div className="rounded-lg bg-surface border border-border p-4 md:p-5">
          <div className="flex flex-col md:flex-row md:items-end gap-6">
            ...
            <div className="flex gap-2 md:flex-col lg:flex-row">
              ...
              <Button size="lg" variant="outline" onClick={openTestModal}>
                <Send className="h-4 w-4 mr-1.5" /> {isLive ? 'Send Request' : 'Send Test Request'}
              </Button>
            </div>
          </div>
      ...
      <Dialog open={testOpen} onOpenChange={setTestOpen}>
        <DialogContent className="bg-surface border-border max-w-lg">
          <DialogHeader>
            <DialogTitle>{isLive ? 'Send Request' : 'Send Test Request'}</DialogTitle>
            <DialogDescription>
              {isLive
                ? `Submit a real, paid request to ${agent.name}. The agent will be matched and you can track the result here.`
                : `Free sandbox call to ${agent.name}. No sats charged. Output is mocked.`}
            </DialogDescription>
          </DialogHeader>

          {/* Live mode: identity guard */}
          {isLive && !identityPubkey && (
            <div className="rounded-lg border border-warning/30 bg-warning/10 p-3 flex items-start gap-2 text-xs">
              ...
              <div>
                <p className="font-semibold text-warning">No identity set</p>
                <p className="text-muted-foreground mt-0.5">
                  Set your pubkey in <Link to="/dashboard" className="underline">Dashboard → Settings</Link> to send live requests.
                </p>
              </div>
            </div>
          )}
          ...
          <div className="space-y-3">
            {/* Live mode: capability + budget controls */}
            {isLive && agent.skills.length > 0 && (
              <div className="grid sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    className="w-full mt-2 h-10 px-3 bg-surface-2 border border-border rounded-md text-sm font-mono"
                  >
                    {agent.skills.map(c => <option key={c} value={c}>{c}</option>)}
                  </select>
                </div>
                <div>
                  <Label className="text-xs uppercase tracking-wider text-muted-foreground">Budget (sats)</Label>
                  <Input
                    type="number"
                    value={budgetInput}
                    min={minBudget}
                    onChange={e => setBudgetInput(Math.max(0, +e.target.value))}
                    className="mt-2 font-mono"
                  />
                  <p className="text-[10px] text-muted-foreground mt-1">
                    Min {minBudget} sats for this capability.
                  </p>
                </div>
              </div>
            )}
            ...
            <div>
              ...
              <Textarea
                ...
                placeholder={
                  isLive
                    ? 'JSON object (e.g. {"text":"hello world"}) — or plain text, which we wrap as { "text": "..." }'
                    : `Try: "${Object.keys(agent.inputSchema)[0] ?? 'input'} = sample value"`
                }
                rows={5}
            ...
            {/* Status / request id */}
            {isLive && testRequestId && (
              <div className="rounded-md border border-border bg-surface-2 p-3 text-xs space-y-1">
                <div className="flex items-center justify-between">
                  <span className="text-muted-foreground uppercase tracking-wider text-[10px]">Request</span>
                  <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium border ${phaseStyle(testPhase)}`}>
                    {testPhase.replace(/_/g, ' ')}
                  </span>
                </div>
                <div className="font-mono break-all text-[11px]">{testRequestId}</div>
                {testRunning && (
                  <div className="flex items-center gap-1.5 text-muted-foreground">
                    <Loader2 className="h-3 w-3 animate-spin" /> Polling for result…
                  </div>
                )}
              </div>
            )}
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={testRunning || (isLive && !identityPubkey)}
              className="bg-primary hover:bg-primary/90"
            >
              {testRunning
                ? <><Loader2 className="h-4 w-4 mr-1.5 animate-spin" />{isLive ? 'Working…' : 'Running…'}</>
                : <><Send className="h-4 w-4 mr-1.5" />{isLive ? 'Submit request' : 'Run test'}</>}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
...
function phaseStyle(phase: RequestPhase): string {
  switch (phase) {
    case 'completed':
